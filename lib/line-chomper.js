...
function mapLineOffsets(source, resolution, lineCallback, callback) {
	if (libTools.isFunction(resolution)) {
		callback = lineCallback;
		lineCallback = resolution;
		resolution = null;
	}
	...
	resolution = resolution || libVars.DEFAULTS.mapOffsetsResolution;

	var currentSectionFrom = resolution,
		lineCount = 0,
		results = [];

	return chomp(source, localLineCallback, completedCallback);

	function localLineCallback(line, offset, sizeInBytes) {
		...
		if (lineCallback) {
			lineCallback(line, offset, sizeInBytes);
		}
	}
